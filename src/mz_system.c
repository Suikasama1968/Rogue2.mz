...
;    ld      (hl), a          // BANK_NUM = 0
    ...
    out     (0xe0), a         // 0x0000-0x0fffをDRAMへ切替
...
push hl                    ; Save the __z88dk_fastcall src argument
call _BANK_VRAM
pop  hl
...
/*
    ...
    使用方向:
    dzx0_decompress_fastcall(src, dst);
        src: 圧縮データ先頭, dst: 展開先
*/
void dzx0_decompress_fastcall(const void *src, void *dst) __naked
{
__asm
    ld      hl, 2
    add     hl, sp               // 引数の位置へ移動
    ...
    inc     hl
    ld      d, (hl)
    pop     hl                   // hl = src address, de = dst address
